...
public class hw14 {
    public class EmployeeApiTest {
        ...
        public void testEmployeeApi() {
            ...
            // Verify "Tiger Nixon" and "Garrett Winters" are among the employees
            ...
            int maxAge = response.jsonPath().getList("data.employee_age").stream().mapToInt(Integer::parseInt).max().orElse(0);
            assertEquals(66, maxAge);

            // Find the employee with the lowest age
            String employeeWithLowestAge = response.jsonPath().getList("data.employee_age").stream()
                    .filter(age -> Integer.parseInt((String) age) == 19)
                    .map(age -> response.jsonPath().getString("data.find { it.employee_age == '" + age + "' }.employee_name"))
                    .findFirst()
                    .orElse(null);
            assertEquals("Tatyana Fitzpatrick", employeeWithLowestAge);

            // Calculate the total salary of all employees
            int totalSalary = response.jsonPath().getList("data.employee_salary").stream().mapToInt(Integer::parseInt).sum();
